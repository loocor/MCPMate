...
const conceptTranslations: Record<Language, Record<string, string>> = {
	...
	ja: {
		"MCPMate: Progressive MCP management partner": "MCPMate：プログレッシブMCP管理パートナー",
		"Import once, expose tools per client, and keep secrets on this machine.": "一度取り込んでクライアントごとにツールを出し、鍵は本機に置く。",
	},
	zh: {
		...
		"MCPMate: Progressive MCP management partner": "MCPMate：渐进式 MCP 管理伙伴",
		"Import once, expose tools per client, and keep secrets on this machine.": "一次导入，按客户端出工具，密钥不出本机。",
...
function isLocalBoardDemoUrl(url: string): boolean {
	try {
...
function boardScreenshotForRoute(routeKey: string): string {
	const shots: Record<string, string> = {
		onboarding: "/screenshot/dashboard-light.png",
		overview: "/screenshot/dashboard-light.png",
		profiles: "/screenshot/profiles-light.png",
		clients: "/screenshot/clients-light.png",
		servers: "/screenshot/servers-light.png",
		market: "/screenshot/market-light.png",
		runtime: "/screenshot/runtime-light.png",
		"api-docs": "/screenshot/dashboard-light.png",
		security: "/screenshot/settings-general-light.png",
	};
	return shots[routeKey] ?? "/screenshot/dashboard-light.png";
}
function buildBoardDemoRouteUrl(route: BoardDemoRoute, baseUrl: string): string {
...
function BoardDemoEmbed() {
	const demoBase = resolveBoardDemoUrl();
	...
	const demoUrl = demoBase ? buildBoardDemoRouteUrl(selectedRoute, demoBase) : "";
				...
				>
					...
					{frameStatus === "unavailable" ? (
						<div className="h-full w-full overflow-hidden">
							<img
								src={boardScreenshotForRoute(selectedRoute.key)}
								alt="MCPMate board"
								className="h-[calc(100%+2.75%)] w-full object-cover object-left-top -translate-y-[2.75%]"
							/>
						</div>
					) : null}
...
export default function Homepage() {
	...
	useEffect(() => {
		setDocumentMeta({
			title: text("MCPMate: Progressive MCP management partner"),
			description: text("Import once, expose tools per client, and keep secrets on this machine."),
			pathname: "/",
		});
